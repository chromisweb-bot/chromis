...
def _devic(nod, a, b, n):
    """
    Equacao de Devic et al. (padrao em dosimetria de filme):
        Dose = a * NOD + b * NOD^n
    Passa naturalmente por (0,0), monotonica crescente.
    n tipicamente entre 2.4 e 2.9 para EBT3/EBT4 (Devic usa 2.5).
    """
    nod_safe = np.clip(nod, a_min=0.0, a_max=None)
    return a * nod_safe + b * np.power(nod_safe, n)
    ...
    if model_type == "spline":
        ...
        if initial_guess is None:
            ...
            if model_type == "power_law":
                ...
                # a ~ dose/nod inicial, b ~ termo nao-linear, n=2.5 (Devic)
                dmax = float(np.max(dose)) if len(dose) else 1000.0
                nmax = float(np.max(nod)) if len(nod) else 1.0
                a0 = dmax / nmax if nmax > 1e-6 else 1000.0
                initial_guess = np.array([a0 * 0.5, a0 * 0.5, 2.5])
