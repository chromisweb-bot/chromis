"""gamma_view.py — Tela do módulo (interface pronta; cálculo em integração)."""
...
def gamma_view(state, go):
    ...
    with col_main:
        st.markdown(f"<div style='background:{COLORS['bg_card']};border:0.5px solid {COLORS['border_soft']};"
                    ...
                    f"Área de visualização — {t('mod_gamma')}</div>", unsafe_allow_html=True)
    with col_side:
        st.markdown(f"**{t('mod_gamma')}**")
